...
const {firstName,lastName,email,password,date,adress,gender,telephone }=req.body
console.log(req.body)

const user = await User.findOne({email})
console.log(user)
if (user) return res.status(400).json({errors:[{msg:'user already exist!!!'}]})
const newUser = new User({firstName,lastName,email,password,date,adress,gender,telephone})
//cryptage password
